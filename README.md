...
|--------|-------|------------|
| `GET` | `/health` | Проверяет доступность cost service. |
| `GET` | `/livez` | Liveness probe контейнера. |
| `GET` | `/service-info` | Возвращает служебную информацию о cost service. |
| `GET` | `/costs/summary` | Возвращает сводку затрат за период: всего, по моделям, командам, пользователям и кластерам. |
| `GET` | `/costs/history` | Возвращает временной ряд стоимости инференса и энергопотребления. |
| `GET` | `/costs/model-rates` | Возвращает ручные тарифы моделей за входные и выходные токены. |
| `PUT` | `/costs/model-rates/{model_name:path}` | Создает или обновляет тариф конкретной модели. |
| `GET` | `/costs/electricity-price` | Возвращает текущую цену электроэнергии для автоматического расчета. |
| `PUT` | `/costs/electricity-price` | Обновляет цену электроэнергии в рублях за кВт·ч. |
| `POST` | `/costs/usage-events` | Записывает событие стоимости по завершенному inference-запросу. |
